...
/* Temperature task */
#define TEMP_STACK_SIZE   (128)
struct os_task g_temp_task;
OS_TASK_STACK_DEFINE(g_temp_stack, TEMP_STACK_SIZE);

/* Callback function for temp task event */
static void temp_cb(struct os_event *);
...
/* push a new tempature sample to the circular queue */
static void
...
/* init temp module and fire an event after delay is complete */
void
temp_task(void *arg)
{
    os_time_t osticks;
    os_time_ms_to_ticks(100, &osticks);

    init_temp();

    while(1) {
        os_time_delay(osticks);
...
main(void)
{
    ...
    /* Init the temp task */
    os_task_init(&g_temp_task, "temp", temp_task, NULL,
                  5, OS_WAIT_FOREVER, g_temp_stack,
                  TEMP_STACK_SIZE);
